...
public class WorkoutTemplateService(WorkoutServiceDbContext dbContext, IMapper mapper) : IWorkoutTemplateService
{
  ...
  private static DifficultyEnum ComputeOverallDifficulty(List<WorkoutTemplateExercise> workoutTemplateExercises)
  {
    if (workoutTemplateExercises.Any(e => e.Exercise is null))
      throw new InvalidOperationException("One or more workout template exercises have a null Exercise reference.");

    return workoutTemplateExercises.Max(e => e.Exercise!.Difficulty);
  }
